...
batch_size=100
experiment_batch_name=trad_vt_electricity
experiment_prepend=trad_vt_electricity
...
for dim in $(seq 0 320); do
    for horizon_length in 96 192 336 720; do
        experiment="${experiment_prepend}_electricity_${history_length}_${horizon_length}_d_${dim}"
        run_experiment
    done
done
